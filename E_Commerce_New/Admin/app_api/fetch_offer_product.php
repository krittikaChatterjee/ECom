<?php
    ...
    $product_query = mysqli_query($conn,"SELECT * FROM `product` WHERE `offer_product`='Y' AND `status`='Y' ORDER BY `product_id` DESC LIMIT 6");
    ...
    if($product_row != 0){
    	while($product_result = mysqli_fetch_assoc($product_query)){
    		...
    		'qty_details'=>$qty_array,'review'=>$avg_review,'exp_status' => $product_result['exp_status'],'rating'=>$rating,));
    		$qty_array = [];
    	}
