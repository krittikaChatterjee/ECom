<?php
    ...
    $sql = "SELECT related_product_id FROM related_product WHERE product_id='$p_id'";
    $result = $conn->query($sql);
    
    if ($result->num_rows > 0) {
      // output data of each row
      while($row = $result->fetch_assoc()) {
        $re_product=$row['related_product_id'];
            $product_query = mysqli_query($conn,"SELECT * FROM `product` WHERE `product_id`='$re_product' AND `status`='Y'");
    ...
    if($product_row != 0){
    	while($product_result = mysqli_fetch_assoc($product_query)){
    		...
    		'persentage_of'=>$product_result['persentage_of'],'main_image'=>$product_result['main_image'],
    		'add_by'=>$product_result['vender_or_admin'],'stock_status'=>$product_result['stock_status'],
    		'qty_details'=>$qty_array,'rating'=>$rating,'exp_status' => $product_result['exp_status']));
    		$qty_array = [];
    	}
