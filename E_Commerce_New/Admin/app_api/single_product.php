<?php
    ...
    $qty_array = array();
      $img_array = array();
    $sub_cat_id = mysqli_real_escape_string($conn,$_REQUEST['p_id']);
    $user_id = mysqli_real_escape_string($conn,$_REQUEST['user_id']);
    
    $product_query = mysqli_query($conn,"SELECT * FROM `product` WHERE `product_id`='$sub_cat_id' AND `status`='Y'");
    ...
    if($product_row != 0){
    	while($product_result = mysqli_fetch_assoc($product_query)){
    		...
    		$total_rating = 0;
    			...
    			array_push($img_array,array('image'=>$product_result['main_image']));
             	array_push($img_array,array('image'=>$product_result['sub_img1']));
            	array_push($img_array,array('image'=>$product_result['sub_img2']));
            	array_push($img_array,array('image'=>$product_result['sub_img3']));
            	array_push($img_array,array('image'=>$product_result['sub_img4']));
    		
    		array_push($array,array('p_id'=>$product_result['product_id'],'product_name'=>$product_result['product_name'],
    		'main_image'=>$img_array,'product_desc'=>$product_result['product_description'],
    		'stock_status'=>$product_result['stock_status'],'qty_details'=>$qty_array,
