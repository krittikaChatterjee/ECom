		...
		<div class="container">
			...
			<div class="product-list-view">
				...
				// echo    $pagination_qry = "SELECT COUNT(qty_per_unit.product_id) AS total_product_id FROM `qty_per_unit` LEFT JOIN `product` ON qty_per_unit.product_id = product.product_id WHERE product.status = 'Y' AND product.featured_product = 'Y'";
                    ...
                    $product_pagination = mysqli_query($conn,"SELECT COUNT(qty_per_unit.product_id) AS total_product_id FROM `qty_per_unit` LEFT JOIN `product` ON qty_per_unit.product_id = product.product_id WHERE product.status = 'Y' AND product.bestseller_product = 'Y'");
                    ...
                    $next = $page + 1;
                        ...
                        $featre_qtyPerUnit_sql = mysqli_query($conn,"SELECT * FROM `qty_per_unit` LEFT JOIN `product` ON qty_per_unit.product_id = product.product_id WHERE product.status = 'Y' AND product.bestseller_product = 'Y' LIMIT $start,$limit");
				...
				<nav aria-label="Page navigation example">
                    ...
                    <?php } else { ?>
                    <li class="page-item"><a class="page-link" href="best_selling_prods.php?page=<?=$previous?>"><i class="fa fa-angle-left"></i></a></li>
                    <?php }
                    for($i=1;$i<=$pages;$i++){
                    ?>
                    <li class="current page-item"><a class="page-link" href="best_selling_prods.php?page=<?=$i?>"><?=$i?></a></li>
                    <?php } 
                    if($page==$pages){
                    ?>
                    <li class="disabled page-item"><a class="page-link" href="#"><i class="fa fa-angle-right"></i></a></li>
                    <?php } else { ?>
                    <li class="page-item"><a class="page-link" href="best_selling_prods.php?page=<?=$next?>"><i class="fa fa-angle-right"></i></a></li>
                    <?php } ?>
                    </ul>
                </nav>
			</div>
